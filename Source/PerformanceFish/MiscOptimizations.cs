...
public sealed class MiscOptimizations : ClassWithFishPatches
{
	...
	public sealed class CompRottable : FishPatch
	{
		...
		// RimWorld 1.6 moved comps to the interval-tick model: CompRottable no longer
		// overrides CompTick, it overrides CompTickInterval(int). AccessTools.Method walks
		// the hierarchy, so asking for "CompTick" silently resolved to the ABSTRACT
		// ThingComp.CompTick, and Harmony refused it at startup with:
		//   "You can only patch implemented methods/constructors."
		// DeclaredMethod restricts the search to the type itself, so this now either finds
		// the real override or returns null - it can no longer bind to the wrong method.
		public override MethodBase TargetMethodInfo { get; }
#if V1_6
			= AccessTools.DeclaredMethod(typeof(RimWorld.CompRottable),
				nameof(RimWorld.CompRottable.CompTickInterval), [typeof(int)]);
#else
			= AccessTools.DeclaredMethod(typeof(RimWorld.CompRottable),
				nameof(RimWorld.CompRottable.CompTick));
#endif
...
#if V1_6
		// Signature must match the patched method: CompTickInterval(int delta).
		// The throttle still fires every 256 ticks, but now forwards the real delta so
		// rot maths stays correct rather than assuming a fixed 256.
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
