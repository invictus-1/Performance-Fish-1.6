...
public sealed class Haulables : ClassWithFishPrepatches
{
	...
	public record struct Cache()
	{
		...
		// RimWorld 1.6 changed ListerHaulables.haulables from List<Thing> to HashSet<Thing>.
		// Kept behind a conditional so the 1.4 and 1.5 projects still compile unchanged.
		// HashSet<T> exposes a struct enumerator, so foreach here allocates nothing - which
		// matters in a mod whose whole purpose is avoiding per-tick garbage.
#if V1_6
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void CheckUpdate(HashSet<Thing> haulables)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public bool IsDirty(HashSet<Thing> haulables) => haulables.Count != ContainedThings.Count;

		[MethodImpl(MethodImplOptions.NoInlining)]
		public void Update(HashSet<Thing> haulables)
		{
