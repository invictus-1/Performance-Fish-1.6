...
public class FishPrepatchHolder : IExposable, IEnumerable<FishPrepatchBase>
{
	...
	{
		// TEMPORARY BISECT SWITCH - same pf-skip.txt used by InitializeAllPatchClasses, but
		// applied to INDIVIDUAL nested patches too. Without this the skip list can only turn
		// off whole classes like Listers.Buildings, which bundles ten separate patches and
		// makes it impossible to tell which one is at fault.
		// Match on the nested name, e.g.  Buildings+AddPatch  or just  AddPatch
		if (PerformanceFishMod.ShouldSkip(type))
		{
			Verse.Log.Message($"[PF-SKIP] patch {type.FullName}");
			return;
		}
