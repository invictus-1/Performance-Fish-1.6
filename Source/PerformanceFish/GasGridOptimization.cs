...
public sealed class GasGridOptimization : ClassWithFishPrepatches
{
	...
	public sealed class SetDirectPatch : FishPrepatch
	{
		...
		// RimWorld 1.6 has FOUR SetDirect overloads - (IntVec3|int) x (4 density bytes|packed).
		// A name-only lookup throws AmbiguousMatchException, which is what killed this patch
		// at startup. Name the parameter types explicitly.
		public override MethodBase TargetMethodBase { get; }
#if V1_4
			= AccessTools.DeclaredMethod(typeof(GasGrid), nameof(GasGrid.SetDirect),
				[typeof(int), typeof(byte), typeof(byte), typeof(byte)]);
#else
			= AccessTools.DeclaredMethod(typeof(GasGrid), nameof(GasGrid.SetDirect),
