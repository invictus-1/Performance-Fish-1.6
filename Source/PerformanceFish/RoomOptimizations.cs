...
public sealed class RoomOptimizations : ClassWithFishPatches, IHasDescription
{
	...
	public sealed class ContainedAndAdjacentThings_Patch : FirstPriorityFishPatch
	{
			...
			ref CacheValue cache)
		{
			...
			// 1.6 allocates RegionGrid.regionGrid lazily; the field is null until first
			// touch. DirectGrid is the accessor that forces allocation.
#if V1_6
			var regionGrid = map.regionGrid.DirectGrid;
#else
			var regionGrid = map.regionGrid.regionGrid;
#endif
			var mapSizeX = map.cellIndices.sizeX;
...
#if V1_6
			var region = map.regionGrid.DirectGrid[(c.z * cellIndices.sizeX) + c.x];
#else
