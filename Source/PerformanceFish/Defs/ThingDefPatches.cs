...
public sealed class ThingDefPatches : ClassWithFishPrepatches
{
	...
	public sealed class GetHashCodePatch : FishClassPrepatch
	{
		...
		{
			// RimWorld 1.6 removed the GetHashCode override from BuildableDef - it now only
			// exists on Def. The original code did `GetMethod(...)!.Body` and null-crashed.
			// Bail out before touching anything rather than half-applying the patch: the
			// field insert and constructor rewrite below must not happen if we cannot also
			// rewrite GetHashCode to read that field.
			var getHashCodeMethod = typeDefinition.GetMethod(nameof(BuildableDef.GetHashCode));
			if (getHashCodeMethod is null)
			{
				Verse.Log.Warning($"[Performance Fish] {typeDefinition.FullName} does not declare "
					+ "GetHashCode. Skipping the def hashcode cache. This is a lost optimization, "
					+ "not a broken game.");
				return;
			}
			...
			var getHashCodeBody = getHashCodeMethod.Body;
