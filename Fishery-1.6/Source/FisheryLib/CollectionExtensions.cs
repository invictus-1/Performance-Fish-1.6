...
public static class CollectionExtensions
{
	...
	/// <summary>
	/// Returns the index of the first entry in the bucket for <paramref name="keyCode"/>,
	/// or a negative value when the bucket is empty.
	/// </summary>
	/// <remarks>
	/// RimWorld 1.6 ships the .NET Core 2.1+ Dictionary layout, in which _buckets stores
	/// (entryIndex + 1) and 0 means "empty bucket". Every earlier version stored the raw
	/// entry index with -1 for empty. Reading the new layout with the old convention makes
	/// an empty bucket look like entry 0 and shifts every chain by one - which walks the
	/// wrong entries and can loop forever. That was the RimWorld 1.6 startup hang.
	///
	/// Callers must pair this with `while ((uint)bucket &lt; (uint)dictionary._entries.Length)`
	/// rather than `while (bucket >= 0)`, so that both the -1 and the 0-1 = -1 cases
	/// terminate, and so a corrupt index cannot run off the end of the array.
	/// </remarks>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static int GetFirstEntryIndex<TKey, TValue>(this Dictionary<TKey, TValue> dictionary, int keyCode)
#if V1_6
		=> dictionary._buckets[(int)((uint)keyCode % (uint)dictionary._buckets.Length)] - 1;
#else
		=> dictionary._buckets[keyCode % dictionary._buckets.Length];
#endif
	...
	public static ref TValue GetReference<TKey, TValue>(this Dictionary<TKey, TValue> dictionary, TKey key)
	{
		if (dictionary._buckets != null)
		{
			...
			var bucket = dictionary.GetFirstEntryIndex(keyCode);
			while ((uint)bucket < (uint)dictionary._entries.Length)
			{
