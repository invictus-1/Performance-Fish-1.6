...
#if V1_6
				// EquatableReferenceType<T> is NEVER selected on 1.6. Its body compiles to a
				// `constrained. callvirt IEquatable<T>.Equals(T)`, and for reference types
				// the runtime shares one code body across every T (__Canon). Resolving that
				// constrained call requires the hidden generic-context argument, which a raw
				// delegate*<T,T,bool> call does not pass - so it jumps to an invalid address
				// and takes the process down. That was the crash-to-desktop when drop pod
				// contents spawned: ListerThings.Add -> IndexedFishSet.Add ->
				// FishTable<Pawn,int>.TryAdd -> here.
				//
				// Requiring IsValueType makes reference types fall through to Object<T>,
				// which does a plain callvirt object.Equals and needs no generic context.
				// For any well-behaved IEquatable<T> implementation that is the same answer,
				// because Equals(object) forwards to Equals(T).
				//
				// EquatableValueType<T> is unaffected: value types get their own specialised
				// code body per T, so no hidden argument is involved.
				?? (type.IsValueType && type.IsAssignableTo(typeof(IEquatable<>), type)
					? GetGeneric(nameof(FunctionPointers.Equals.Methods.EquatableValueType), type)
#else
